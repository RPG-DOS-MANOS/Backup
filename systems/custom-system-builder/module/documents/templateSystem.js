 ...
 */
import { getGameCollection, getGameCollectionAsTemplateSystems, removeEmpty } from '../utils.js';
...
class TemplateSystem {
    ...
    _prepareEntityData() {
        ...
        const attributeBars = {
            ...Object.keys(system.attributeBar ?? {}).reduce((res, key) => ((res[key] = new SimpleAttributeBarElement(key, String(system.attributeBar[key].value ?? ''), system.attributeBar[key].max)),
                res), {}),
        ...
        for (const component in computableComponents) {
            computeFormulas = {
                ...computeFormulas,
                ...computableComponents[component].getComputeFunctions(this, modifierPropsByKey)
            };
        }
        for (const prop of Object.keys(computeFormulas)) {
        ...
        do {
            ...
            for (const prop in uncomputedProps) {
                try {
                    const newComputedValue = uncomputedProps[prop](computedProps);
    ...
    async roll(rollKey, options = {}) {
        ...
        const allRolls = this.getCustomRolls();
        if (!allRolls) {
            throw error;
        }
        const chatSenderFunction = foundry.utils.getProperty(allRolls, `${rollKey}.${rollType}`);
        if (typeof chatSenderFunction !== 'function') {
            throw error;
        }
    ...
    getCustomRolls() {
        const customRolls = {};
        Object.keys(this.componentMap)
            .filter((key) => isChatSenderElement(this.componentMap[key]))
            .forEach((key) => {
            const chatSenderElt = this.componentMap[key];
            foundry.utils.mergeObject(customRolls, chatSenderElt.getSendToChatFunctions(this));
        });
        return customRolls;
    }
    /**
    ...
    getModifiers() {
        ...
        for (const item of this.items) {
            ...
            if (!itemTemplate) {
                const warnMsg = `Item template has been deleted for item ${item.name} - ${item.uuid} used in ${this.entity.name} - ${this.uuid}`;
                Logger.warn(warnMsg);
        ...
        if (template.system.attributeBar) {
            for (const barName in this.system.attributeBar) {
                if (!template.system.attributeBar[barName]) {
                    template.system.attributeBar['-=' + barName] = { max: '0' };
                }
            }
        }
    ...
    activateListeners(html) {
        ...
        if (this.isTemplate) {
                ...
                templateFunctions.attributeBars((newAttributeBars) => {
                    ...
                    for (const barName in this.system.attributeBar) {
                        if (!newAttributeBars[barName]) {
                            newAttributeBars['-=' + barName] = { max: '0' };
                        }
                    }
                    // Update the entity with new hidden attributes
                    this.entity
            ...
            html.find('.custom-system-configure-active-effects').on('click', (_ev) => {
                const allEffects = CONFIG.statusEffects.map((anEffect) => {
                    const newEffect = { ...anEffect, modifiers: [], label: '' };
    ...
    async openAttributesVision() {
        ...
        const content = await renderTemplate(`systems/${game.system.id}/templates/_template/dialogs/readAttributes.html`, {
            attributes
        });
        new CustomDialog({
            title: `Actor's hidden attributes`,
            content,
            buttons: {
                ok: {
                    label: 'OK'
    ...
    async openAttributeBarsVision() {
        ...
        const content = await renderTemplate(`systems/${game.system.id}/templates/_template/dialogs/readAttributeBars.html`, {
            attributeBars
        });
        new CustomDialog({
            title: `Actor's hidden attributes`,
