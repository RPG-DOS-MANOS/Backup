...
export default class ComputablePhrase {
    ...
    async compute(props, options = { computeExplanation: false, availableKeys: [] }) {
        ...
        options.updates = options.updates ?? { actors: {}, items: {} };
        ...
        if (Object.keys(options.updates?.actors).length > 0) {
            Logger.debug('Updating Actors', options.updates.actors);
            await Actor.updateDocuments(Object.values(options.updates.actors));
        }
        if (Object.keys(options.updates?.items).length > 0) {
            const noParentItem = [];
            const itemByParent = {};
            const allParents = {};
            for (const item of Object.values(options.updates.items)) {
