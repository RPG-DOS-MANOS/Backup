...
class TabbedPanel extends Container {
     ...
     */
    constructor({ key, tooltip = null, templateAddress, contents = [], cssClass = null, role = 0, permission = 0, visibilityFormula = null, parent = null }) {
        super({
            key: key,
            tooltip: tooltip,
    ...
    async _getElement(entity, isEditable = true, options = {}) {
        ...
        if (entity.isTemplate) {
            ...
            addTabButton.on('click', () => {
                ...
                templateFunctions.editTab(({ name, key, role = 0, permission = 0, visibilityFormula = null, tooltip = null }) => {
     ...
     * @param {ContainerJson?} existingComponent
     * @param {TemplateSystem} entity
     * @return {Promise<JQuery>} The jQuery element holding the component
     */
    static async getConfigForm(existingComponent, entity) {
        let mainElt = $('<div></div>');
        mainElt.append(await renderTemplate(`systems/${game.system.id}/templates/_template/components/tabbed-panel.html`, existingComponent));
        return mainElt;
    }
    /**
     ...
     * @return {ContainerJson} The JSON representation of the component
