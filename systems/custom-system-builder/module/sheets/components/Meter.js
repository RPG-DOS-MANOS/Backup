...
const METER_TEXT_OPTIONS = {
    none: 'None',
    showVal: 'Show value',
    showMax: 'Show value and maximum',
    showPercent: 'Show value as percentage'
};
...
class Meter extends InputComponent {
        ...
        if (entity.isTemplate) {
            ...
            switch (this._textOption) {
                ...
                case 'showMax':
                    ...
                    break;
                case 'showPercent':
                    meterText.text(`${Math.roundDecimals(meterPercent, 2)}%`);
                    break;
            }
        }
    ...
    getComputeFunctions(entity, modifiers, options, keyOverride) {
        ...
        return {
            [computationKey]: (additionalProps) => {
                ...
                let value = parseFloat(ComputablePhrase.computeMessageStatic(this._value ?? '', formulaProps, {
                    ...options,
                ...
                if (modifiers[computationKey]) {
                    value = parseFloat(applyModifiers(value, modifiers[computationKey])?.toString() ?? '');
                }
                return value;
            }
        };
    ...
    static async getConfigForm(existingComponent, _entity) {
        ...
        mainElt.append(await renderTemplate(`systems/${game.system.id}/templates/_template/components/meter.html`, {
