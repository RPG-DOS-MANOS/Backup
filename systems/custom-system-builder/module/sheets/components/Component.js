...
class Component {
    ...
    async _getElement(entity, _isEditable = true, options = {}) {
        ...
        if (this.tooltip) {
            ...
            if (!entity.isTemplate) {
                try {
                    tooltipText = (await ComputablePhrase.computeMessage(this.tooltip, {
                        ...entity.system.props,
                        ...options.customProps
                    }, {
                        ...options,
    ...
    static extractConfig(html) {
        ...
        if (fieldData.key && !fieldData.key.match(/^[a-zA-Z0-9_]+$/)) {
            throw new Error('Component key must be a string composed of upper and lowercase letters and underscores only.');
        }
        return fieldData;
    }
}
