...
const editTab = async (callback, tabData = null) => {
    ...
    let content = await renderTemplate(`systems/${game.system.id}/templates/_template/dialogs/edit-tab.html`, {
        ...tabData,
...
const component = async (callback, { componentData = undefined, allowedComponents = undefined, isDynamicTable = false, entity = undefined }) => {
    ...
    let content = await renderTemplate(`systems/${game.system.id}/templates/_template/dialogs/component.html`, {
        ...componentData,
            ...
            delete: {
                ...
                callback: () => {
                    Dialog.confirm({
                        ...
                        content: '<p>Are you sure you want to delete this component ?</p>',
                        yes: () => {
...
const attributes = async (callback, attr) => {
    // Render the dialog's contents
    let content = await renderTemplate(`systems/${game.system.id}/templates/_template/dialogs/attributes.html`, {
        attribute: attr
    });
...
const attributeBars = async (callback, attr) => {
    ...
    // Render the dialog's contents
    let content = await renderTemplate(`systems/${game.system.id}/templates/_template/dialogs/attributeBars.html`, {
        attribute: attr
    });
...
const displaySettings = async (callback, attr) => {
    // Render the dialog's contents
    let content = await renderTemplate(`systems/${game.system.id}/templates/_template/dialogs/display.html`, attr);
...
const modifiers = async (callback, blocks) => {
    // Render the dialog's contents
    let content = await renderTemplate(`systems/${game.system.id}/templates/_template/dialogs/modifiers.html`, {
        blocks
    });
    ...
    modifiersDialog = new Dialog({
        ...
        render: (html) => {
            ...
            html.find('.custom-system-modifiers #addModifier').on('click', (ev) => {
                ...
                const lastRow = target.parents('tr');
                // Create new row
                const newRow = $(`<tr class="custom-system-modifier">
    <input type="hidden" class="custom-system-modifier-id" value="${foundry.utils.randomID()}"/>
    <td>
        <input type="text" class="custom-system-modifier-conditionalGroup">
    </td>
    <td>
        <input type="number" class="custom-system-modifier-priority" value="0" />
    </td>
    <td>
    ...
    <td class="custom-system-modifier-controls">
        <a class="custom-system-delete-modifier">
