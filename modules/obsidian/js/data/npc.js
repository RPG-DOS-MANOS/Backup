...
export function prepareNPCHD (system, flags, derived) {
...
export function prepareSpeed (system) {
...
export function prepareVehicleActions (system, derived) {
	const actions = system.attributes.actions;
	if (!actions.value) {
		return;
	}
...
export function prepareVehicleLayout (actor, flags, derived) {
	...
	for (const item of actor.items) {
		...
		if (i) {
			i.quantity = item.system.quantity;
			i.name = item.name;
			i.img = item.img;
			i.type = 'items';
			layout.groups[i.parent]?.items.push(i);
...
export async function refreshNPC (combat) {
		...
		for (const effect of item.flags.obsidian.effects) {
			for (const component of effect.components) {
				...
				if (recharge.flags.obsidian.addendum.success) {
					...
					itemUpdates.push(
						OBSIDIAN.updateArrays(item._source, {
							_id: item.id,
	...
	if (legact.max && legact.value) {
		actor.update({'system.resources.legact.value': 0});
	}
}
