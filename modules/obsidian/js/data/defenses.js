...
export function hpAfterDamage (actor, damage, attack) {
	...
	const defenses = actor.obsidian.defenses.parts.damage;
	const dr = actor.obsidian.defenses.parts.dr;
	const dt = actor.flags.obsidian.attributes.dt;
	...
	const update = {};
	if (current !== hp.value && (!dt || total >= dt)) {
		update['system.attributes.hp.value'] = Math.clamped(current, 0, hp.max);
	}

	if (hp.temp !== tmp) {
		update['system.attributes.hp.temp'] = tmp;
	}
