...
import {addCreateObjectHooks} from './module/objects.js';
...
import {addChatLogDropHandlers} from './module/chat-log.js';

runPatches();
extendPrimitives();

Hooks.once('init', async function () {
	CONFIG.compatibility.excludePatterns.push(/flags\.core\.statusId/);
	CONFIG.compatibility.excludePatterns.push(/TokenDocument#actorData/);
...
function enrichActorFlags (actor) {
	actor.updateSource(Migrate.convertActor(actor.toObject()));
}

function enrichItemFlags (item) {
...
Hooks.on('preCreateActor', actor => {
	enrichActorFlags(actor);
});

Hooks.on('preCreateItem', item => {
	enrichItemFlags(item);
	item.parent?.linkClasses(item);
});
