...
export const v14 = {
	...
	convertSpellcasting: function (data) {
		if (data.flags.obsidian?.spellcasting?.enabled && data.system.spellcasting) {
			data.system.spellcasting.ability = data.flags.obsidian.spellcasting.spell;
		}
	},

	convertCreatureType: function (data) {
		if (data.flags.obsidian.details?.type) {
			data.system.details.type.value = data.flags.obsidian.details.type;
		}

		if (data.flags.obsidian.details?.tags?.custom) {
			data.system.details.type.subtype = data.flags.obsidian.details.tags.custom;
		}
	},
	...
	convertTempMaxHP: function (data) {
		...
		data.system.attributes.hp.tempmax = hpMaxMod;
	},
