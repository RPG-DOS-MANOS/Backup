...
import {determineAdvantage, determineMode, Prepare} from '../data/prepare.js';
...
import {getSourceClass} from './item.js';
...
export const Rolls = {
	abilityCheck: function (actor, ability, skill, parts = [], rollMod) {
		if (!skill) {
			...
			parts.push({
				mod: actor.system.abilities[ability].mod,
				name: game.i18n.localize(`OBSIDIAN.AbilityAbbr.${ability}`)
			}, ...actor.obsidian.abilities[ability].rollParts);
		}
	...
	applyDamage: async function (evt) {
		...
		if (message) {
			attack = message.flags.obsidian.damage?.attack;
		}
	...
	applySave: async function (evt, collection) {
		...
		const flags = msg.flags.obsidian;
		...
		const effect = actor?.obsidian.effects.get(flags.effectId);
		const item = actor?.items.get(effect?.parentItem);

		if (actor && effect?.applies.length) {
			conditions =
				effect.applies
					.map(uuid => actor.obsidian.effects.get(uuid))
					.filter(_ => _)
		...
		for (let i = 0; i < tokens.length; i++) {
			...
			if (!autoFail) {
				...
				failed =
					roll < component.dc ||
					conditionsAutoFail(token.actor, {
	...
	create: function (actor, options) {
		...
		if (roll === 'item') {
			...
			return Rolls.toChat(actor, ...Rolls.itemRoll(actor, item, options));
		} else if (roll === 'fx') {
			if (options.effectId === undefined) {
				return;
			}

			const effect = actor.obsidian.effects.get(options.effectId);
			if (!effect) {
				return;
			}

			return Rolls.toChat(actor, ...Rolls.effectRoll(actor, effect, options));
		} else if (roll === 'save') {
			if (!options.save) {
				return;
			}
			...
			if (options.save === 'death') {
				return Rolls.toChat(actor, Rolls.death(actor));
			} else {
				return Rolls.toChat(actor, Rolls.savingThrow(actor, options.save));
			}
		} else if (roll === 'abl') {
			if (!options.abl) {
				return;
			...
			if (options.abl === 'init') {
				return Rolls.toChat(actor, Rolls.initiative(actor));
			} else {
				return Rolls.toChat(actor,
					Rolls.abilityCheck(
			...
			const skill = actor.system.skills[options.skl];
			if (!skill) {
				return;
			}
			...
			const effect = actor.obsidian.effects.get(options.effect);
			if (!effect) {
				return;
			}
			...
			msgs.push({
				flags: {
					obsidian: {
						type: 'dmg',
						title: item.name,
						damage: Rolls.rollDamage(actor, dmg, {item, scaling, scaledAmount})
					}
				}
			});
		}
	...
	death: function (actor) {
		...
		const bonuses = actor.obsidian.filters.bonuses(Filters.appliesTo.deathSaves(mode));
		if (bonuses.length) {
		...
		let tally = system.attributes.death[key] + 1;
		...
		if (result.roll === 20) {
			let hp = system.attributes.hp.value;
			if (hp < 1) {
				hp = 1;
			}
			...
			actor.update({[`system.attributes.death.${key}`]: tally});
		}
	...
	effectRoll: function (actor, effect, options, {name, isFirst = true} = {}) {
		...
		if (item.type === 'spell') {
			scaledAmount = Math.max(0, options.spellLevel - item.system.level);
		}
		...
		let details = item.flags.obsidian.display || item.system.description.value;
		if (desc) {
	...
	getDamageFromMessage: function (msg, hit) {
		const damage = new Map();
		if (!getProperty(msg, `flags.obsidian.damage.${hit}`)?.results.length) {
			return damage;
		}

		const accumulate = (key, value) => damage.set(key, (damage.get(key) || 0) + value);
		msg.flags.obsidian.damage[hit].results.forEach(dmg => accumulate(dmg.type, dmg.total));
		return damage;
	},
	...
	hd: function (actor, rolls, conBonus) {
		const rollMod = Effect.combineRollMods(actor.obsidian.filters.mods(Filters.isHD));
		const parts = rolls.map(([n, d]) => {
			return {mod: 0, ndice: n, die: d};
		});

		const bonuses =
			actor.obsidian.filters.bonuses(Filters.isHD).flatMap(bonus =>
	...
	itemRoll: function (actor, item, options) {
		...
		let upcast;
		if (item.type === 'spell') {
			upcast = Math.max(0, options.spellLevel - item.level);
		}
			...
			return [{
				flags: {
					obsidian: {
						...
						title: item.name,
						item: item,
						details: item.obsidian.display || item.description.value,
						open: true,
						upcast: upcast
					}
				}
	...
	findTool: function (actor, tool) {
		return Object.values(actor.system.tools).find(entry =>
			entry.label?.toLocaleLowerCase() === tool.custom.toLocaleLowerCase());
	},

	getExtraDamage: function (actor, item) {
		if (!actor.obsidian) {
			return [];
		}

		const targets = Array.from(game.user.targets).map(target => target.document.uuid);
		const components =
			actor.effects
				.filter(effect =>
					effect.getFlag('obsidian', 'duration')
					&& (effect.getFlag('obsidian', 'active') ?? [])
						.some(uuid => targets.includes(uuid)))
				.map(duration => actor.obsidian.effects.get(duration.getFlag('obsidian', 'ref')))
				.flatMap(effect => effect?.components ?? [])
				.filter(component => component.type === 'extra-damage');
		...
		for (const component of components) {
			const parentEffect = actor.obsidian.effects.get(component.parentEffect);
			const parentItem = actor.items.get(parentEffect?.parentItem);
			const source = parentItem?.getFlag('obsidian', 'source');

			let cls;
			if (source) {
				cls = getSourceClass(actor, source);
			}

			const prepared = duplicate(component);
			Prepare.calculateDamage(actor, item, prepared, cls);
			damage.push(prepared);
		}
	...
	placeTemplate: function (evt) {
		...
		item.system.target.type = aoe.area;
		item.system.target.value = scaledDistance;
	...
	rollDamage: function (actor, damage, {item = null}) {
		const extraDamage = Rolls.getExtraDamage(actor, item);
		extraDamage.forEach(dmg => dmg.damage ||= damage[0].damage);
		damage.push(...extraDamage);
		damage.push(
		...
		const diceParts = damage.map(dmg => {
			...
			if (dmg.addMods !== false) {
				const rollMods =
					Effect.filterDamage(actor, actor.obsidian.filters.mods, dmg);
	...
	savingThrow: function (actor, save, {conditions = [], item} = {}) {
		...
		if (item) {
			const defenses = actor.obsidian.defenses.parts.conditions;
		...
		return Rolls.simpleRoll(actor, {
			...
			subtitle: game.i18n.localize('OBSIDIAN.SavingThrow'),
			parts: actor.obsidian.saves[save].rollParts,
			rollMod
		});
	},
	...
	toHitRoll: function (actor, hit, extraParts = []) {
		...
		if (!token && canvas) {
			...
			const {linked, unlinked} = canvas.tokens.placeables.reduce((acc, t) => {
				if (!(t instanceof Token) || t.actorId !== actor.id) {
					return acc;
				}
		...
		if (token && game.user.targets.size === 1) {
			...
			if (target.actor) {
				rollMods.push(
					targetConditionsRollMod(target.actor, token.within5ftOf(target)));
			}
		}
