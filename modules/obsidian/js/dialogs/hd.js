...
export class ObsidianHDDialog extends ObsidianDialog {
	...
	async getData () {
		...
		data.hd = duplicate(this.parent.actor.flags.obsidian.attributes.hd);
	...
	async _onAddHD (evt) {
		evt.preventDefault();
		const hd = duplicate(this.parent.actor.flags.obsidian.attributes.hd);
		const existingHD = Object.keys(hd);
		const dice = OBSIDIAN.Config.HD.map(die => `d${die}`);
		const availableHD =
			dice.filter(die => {
