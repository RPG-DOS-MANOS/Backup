...
export class ObsidianViewDialog extends ObsidianDialog {
	constructor (itemID, parent, options = {}) {
		...
		if (item.type === 'backpack') {
			const hook = `renderObsidian${parent.actor.type === 'npc' ? 'NPC' : 'Character'}`;
	...
	async close () {
		if (this._hook) {
			const hook =
				`renderObsidian${this.parent.actor.type === 'npc' ? 'NPC' : 'Character'}`;
		...
		html.find('[draggable]').each((i, row) =>
			row.addEventListener('dragstart', evt => {
				if (evt.currentTarget.classList.contains('obsidian-char-inv-header-currency')) {
					...
					evt.dataTransfer.setData('currency-drop', null);
				}
