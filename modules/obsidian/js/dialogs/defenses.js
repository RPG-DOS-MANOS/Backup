...
export class ObsidianDefensesDialog extends ObsidianDialog {
	...
	async _addBPS (level) {
		...
		const damage = duplicate(this.parent.actor.flags.obsidian.defenses.damage);
		damage.push(
	...
	async _addMagicRes () {
		const update = this._formData;
		const conditions = duplicate(this.parent.actor.flags.obsidian.defenses.conditions);
	...
	async _onAddDamage () {
		...
		update['flags.obsidian.defenses.damage'] =
			duplicate(this.parent.actor.flags.obsidian.defenses.damage);
	...
	async _onRemoveDamage (evt) {
		const update = this._formData;
		update['flags.obsidian.defenses.damage'] =
			ObsidianDialog.removeRow(this.parent.actor.flags.obsidian.defenses.damage, evt);
		await this.parent.actor.update(update);
		this.render(false);
	}
	...
	async _onRemoveCondition (evt) {
		...
		update['flags.obsidian.defenses.conditions'] =
			ObsidianDialog.removeRow(
				this.parent.actor.flags.obsidian.defenses.conditions, evt);
		await this.parent.actor.update(update);
		this.render(false);
	}
}
