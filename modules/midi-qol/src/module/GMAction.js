...
import { canSense, completeItemUse, getToken, getTokenDocument, gmOverTimeEffect, fromActorUuid, MQfromUuidSync, promptReactions, hasUsedAction, hasUsedBonusAction, hasUsedReaction, removeActionUsed, removeBonusActionUsed, removeReactionUsed, isEffectExpired, expireEffects, getAppliedEffects } from "./utils.js";
	...
	for (let damageItem of damageList) {
		...
		if (createPromises && doHits && (["yes", "yesCard", "yesCardNPC", "yesCardMisses"].includes(data.autoApplyDamage) || data.forceApply)) {
			if ((hpDamage || tempDamage || newVitality !== oldVitality) && (data.autoApplyDamage !== "yesCardNPC" || actor.type !== "character")) {
				...
				if (actor.isOwner) {
					const updates = { "system.attributes.hp.temp": newTempHP, "system.attributes.hp.value": newHP };
		...
		if (configSettings.v3DamageApplication) {
			const tooltipList = damageItem.damageItem?.map(di => {
				let allMods = Object.keys(di.active).reduce((acc, k) => {
					if (["saved", "semiSuperSaver", "superSaver"].includes(k))
						return acc;
