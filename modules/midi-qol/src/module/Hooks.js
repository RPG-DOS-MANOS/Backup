...
function registerBaBonusHooks() {
	...
	Hooks.on("dnd5e.preRollAttack", (item, rollConfig) => {
		rollConfig.fastForward && (rollConfig.fastForward = !foundry.utils.getProperty(item, "flags.midi-qol.babonusOptional")?.length);
	...
	Hooks.on("dnd5e.preRollAbilitySave", (actor, rollData, abilityId) => {
		rollData.fastForward && (rollData.fastForward = !foundry.utils.getProperty(actor, "flags.midi-qol.babonusOptional")?.length);
