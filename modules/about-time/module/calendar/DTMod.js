...
export class DTMod {
   ...
   */
    add(increment) {
        ...
        let seconds = Math.floor(this.interval.seconds + (increment.interval.seconds || 0));
        const result = new DTMod({});
        result.interval = { year, month, day, hour, minute, seconds };
        return result;
    }
    /**
    ...
    static timeString(timeInSeconds) {
        ...
        return `${pad(dmhs.interval.hour)}:${pad(dmhs.interval.minute)}:${pad(dmhs.interval.seconds)}`;
    }
    static fromSeconds(seconds) {
        // wont work for pathfinder
        console.warn(`about-time | fromSeconds deprecated use 
    const interval = window.SimpleCalendar.api.secondsToInterval(seconds);
    `);
        //@ts-ignore
