...
*/
export class GamepadModuleManager {
	...
	constructor() {
		...
		Hooks.on("updateUser", async function (user) {
			const gamepadIndex = game[NAMESPACE].Settings.getGamepadIndexForUser(user.id);
			if (gamepadIndex) {
	...
	* @param GamepadModule
	*/
	registerGamepadModule(GamepadModule) {
	...
	enableContextModule(gamepadIndex, focusModuleId) {
		this.enabledContextModules[gamepadIndex] = focusModuleId;
	}
	disableContextModule(gamepadIndex) {
	...
	*/
	updateGamepadModuleInstance() {
		const gamepadConfigs = game[NAMESPACE].Settings.getGamepadConfigs();
	...
	* @param moduleId
	*/
	deleteGamepadModuleInstance(gamepadIndex, moduleId) {
	...
	*/
	...
	_addGamepadModuleInstance(gamepadIndex, moduleId) {
	...
	_getRegisteredGamepadModuleInstance(gamepadIndex, moduleId) {
		if (this.registeredGamepadModuleInstances[gamepadIndex]) {
