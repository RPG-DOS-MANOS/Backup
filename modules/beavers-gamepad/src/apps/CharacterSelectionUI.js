...
export class CharacterSelectionUI {
	name = "beavers-character-selection";
	async process(userId, userInput) {
		const choices = {};
		...
		if (actorId !== null && actorId !== "") {
			game[NAMESPACE].socket.executeAsGM(SOCKET_UPDATE_USER, userId, { "character": actorId }).then(x => {
				if (game["users"].current.id === userId && canvas instanceof Canvas) {
					const token = canvas.tokens?.objects?.children.find(token => game["users"].current.character.id === token["actor"]?.id);
					if (token) {
						token.control();
					}
				}
