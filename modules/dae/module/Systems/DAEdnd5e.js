import { MODULE_ID, daeSpecialDurations, debug, debugEnabled, error, i18n, warn } from "../../dae.js";
...
export class DAESystemDND5E extends CONFIG.DAE.systemClass {
    ...
    static modifyBaseValues(actorType, baseValues, characterSpec) {
        ...
        if (!baseValues["system.attributes.ac.armor"])
            baseValues["system.attributes.ac.armor"] = [0, -1];
        if (!baseValues["system.attributes.ac.shield"])
            baseValues["system.attributes.ac.shield"] = [0, -1];
        ...
        if (!baseValues["system.attributes.hd.max"])
            baseValues["system.attributes.hd.max"] = [0, -1];
        ...
        baseValues["system.attributes.encumbrance.bonuses.encumbered"] = ["", -1];
        baseValues["system.attributes.encumbrance.bonuses.heavilyEncumbered"] = ["", -1];
        baseValues["system.attributes.encumbrance.bonuses.maximum"] = ["", -1];
        baseValues["system.attributes.encumbrance.bonuses.overall"] = ["", -1];
        baseValues["system.attributes.encumbrance.multipliers.encumbered"] = ["", -1];
        baseValues["system.attributes.encumbrance.multipliers.heavilyEncumbered"] = ["", -1];
        baseValues["system.attributes.encumbrance.multipliers.maximum"] = ["", -1];
        baseValues["system.attributes.encumbrance.multipliers.overall"] = ["", -1];
    }
    static modifySpecials(actorType, specials, characterSpec) {
        ...
        if (daeSystemClass.systemConfig.abilities && foundry.utils.getProperty(actorModelSchemaFields, "abilities"))
            Object.keys(daeSystemClass.systemConfig.abilities).forEach(ablKey => {
                specials[`system.abilities.${ablKey}.mod`] = [0, -1];
                specials[`system.abilities.${ablKey}.save`] = [0, -1];
                // specials[`system.abilities.${ablKey}.min`] = [0, -1]; There is no min attribute yet
                specials[`system.abilities.${ablKey}.max`] = [0, -1];
            });
    }
    ...
    static configureLists(daeConfig) {
        ...
        if (foundry.utils.isNewerVersion(systemVersion, "2.99")) {
            ...
            Object.keys(this.customDamageResistanceList).forEach(type => {
                this.traitList[`${type}`] = `${daeSystemClass.systemConfig.customDamageResistanceTypes[type]}`;
                this.traitList[`-${type}`] = `- ${daeSystemClass.systemConfig.customDamageResistanceTypes[type]}`;
            });
        }
        else {
            ...
            Object.keys(this.traitList).forEach(type => {
                this.traitList[`-${type}`] = `- ${this.traitList[type]}`;
            });
        }
        if (foundry.utils.isNewerVersion(systemVersion, "3.1.99")) {
            this.bypassesList = Object.keys(daeSystemClass.systemConfig.itemProperties)
                .filter(key => daeSystemClass.systemConfig.itemProperties[key].isPhysical)
                .map(key => {
                const returnValue = {};
                returnValue[key] = i18n(daeSystemClass.systemConfig.itemProperties[key].label);
                return returnValue;
            });
        }
        else if (daeSystemClass.systemConfig.damageTypes) {
    ...
    static getOptionsForSpec(spec) {
        ...
        if (spec.key === "system.traits.size") {
            //@ts-expect-error
            if (foundry.utils.isNewerVersion(game.system.version, "3.1")) {
    ...
    static daeCustomEffect(actor, change, current, delta, changes) {
        ...
        switch (change.key) {
            ...
            case "system.traits.dv.bypasses":
                if (foundry.utils.isNewerVersion(systemConfig.version, "3.1.99")) {
