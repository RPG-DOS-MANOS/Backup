...
export async function addConditionChange(actor, change, token, effect) {
    ...
    if (change.value.startsWith("Convenient Effect") && effectInterface) {
        const effectName = change.value.split("Convenient Effect: ")[1];
        //@ts-expect-error
        await game.dfreds.effectInterface.addEffect({ effectName, uuid: actor.uuid, origin: effect.uuid, metaData: {} });
    }
    else {
        if (condition.statuses?.length > 1 && !condition?._id) {
            condition._id = condition.id.replaceAll(/[ :,.\+\-\*\&\^\%\$\#\@\!\[\]{}\(\)]/g, "").padEnd(16, "x").slice(-16);
        }
        if (condition) {
            //@ts-expect-error
            const status = await ActiveEffect.implementation.fromStatusEffect(condition.id, { parent: actor });
            status.origin = effect.uuid;
            let returnValue = await (actionQueue.add(actor.createEmbeddedDocuments.bind(actor), "ActiveEffect", [status], { parent: actor, keepId: true }));
            if (effect.addDependents)
                effect.addDependents(returnValue);
            else if (effect.addDependent)
                effect.addDependent(...returnValue);
            return returnValue;
        }
    }
}
export async function removeConditionChange(actor, change, token, effect, context = {}) {
    if (change.value.startsWith("Convenient Effect")) {
        const effectName = change.value.split("Convenient Effect: ")[1];
        //@ts-expect-error
        await game.dfreds?.effectInterface.removeEffect({ effectName, uuid: actor.uuid, origin: effect.uuid });
    }
    const condition = actor.effects.find(ef => ef.origin === effect.uuid && ef.statuses.has(change.value));
    if (condition)
        await (actionQueue.add(actor.deleteEmbeddedDocuments.bind(actor), "ActiveEffect", [condition.id]));
}
...
async function _preCreateActiveEffectRemoveExisting(wrapped, ...args) {
    ...
    try {
        ...
        if (["noneName", "none", "noneNameOnly"].includes(stackable)) {
            ...
            this.parent.deleteEmbeddedDocuments("ActiveEffect", hasExisting.map(ef => ef.id), context);
        }
    }
    catch (err) {
        ...
        try {
            ...
            if (becameSuppressed || becameDisabled || removedChanges.length > 0) {
                ...
                for (let change of changesToDisable) {
                    ...
                    if (change.key === "StatusEffect") {
                        await removeStatusEffectChange(actor, change, tokens, effect, context);
                    }
                }
...
export async function applyNonTransferEffects(item, activate, targets, options = { whisper: false, spellLevel: 0, damageTotal: null, itemCardId: null, critical: false, fumble: false, tokenId: undefined, effectsToApply: [], removeMatchLabel: false, toggleEffect: false, selfEffects: "none" }) {
    ...
    switch (options.selfEffects) {
        case "selfEffectsAlways":
            appliedEffects = item.effects.filter(ae => ae.transfer !== true && ae.flags?.[game.system.id]?.type !== "enchantment" && ae.flags?.dae?.selfTargetAlways).map(ae => {
            ...
            break;
        case "selfEffectsAll":
            appliedEffects = item.effects.filter(ae => ae.transfer !== true && ae.flags?.[game.system.id]?.type !== "enchantment" && (ae.flags?.dae?.selfTargetAlways || ae.flags?.dae?.selfTarget)).map(ae => {
                const data = ae.toObject(false);
                foundry.utils.setProperty(data, "flags.core.sourceId", ae.parent.uuid);
                return data;
            });
...
export function addEffectChange(actor, tokens, effectToApply, item, change, context) {
    ...
    switch (change.key) {
        ...
        case "StatusEffect":
            //@ts-expect-error
            if (CONFIG.ActiveEffect.legacyTransferral === false)
                addStatusEffectChange(actor, change, token, effectToApply, context);
            break;
        default:
...
export function removeEffectChange(actor, tokens, effectToApply, item, change, context) {
    ...
    switch (change.key) {
        ...
        case "StatusEffect":
            //@ ts-expect-error
            // if (CONFIG.ActiveEffect.legacyTransferral === false)
            removeStatusEffectChange(actor, change, tokens, effectToApply, context);
            break;
        case "macro.StatusEffect":
            //@ts-expect-error
            if (CONFIG.ActiveEffect.legacyTransferral === false)
...
export async function createItemHook(...args) {
    ...
    if (rewriteTransferOrigin) {
        ...
        for (let effect of item.effects) {
            if (effect.transfer && effect.getFlag(game.system.id, "type") !== "enchantment") {
...
export function daeInitActions() {
    ...
    async function preCreateActiveEffect(wrapped, data, context, user) {
        await _preCreateActiveEffectRemoveExisting.bind(this)(wrapped, data, context, user);
        return await _preCreateActiveEffect.bind(this)(wrapped, data, context, user);
    }
    ;
