...
import { getCombats } from "./settings.js";
...
export class CombatReadyAnimation extends SettingsAwareEntity {
    ...
    prepare() {
        if (!this.testMode) {
            ...
            let nxtPlayerName = game.user?.name;
            ...
            let selectedToken = (canvas.tokens?.objects?.children.find(e => {
                //@ts-ignore
                return e.controlled;
            })) ?? undefined;
            var testCombatant = new Combatant({
                tokenId: selectedToken?.id ?? '',
                name: (selectedToken == undefined) ? game.user?.name : selectedToken.name,
...
export class AnimationSubSettings extends CombatReadySubSettings {
    ...
    onAnimationTestClick(event) {
        Object.values(ui.windows).forEach(async (window) => {
            //@ts-ignore
            this.windowsStates.set((window.appId), window._minimized);
